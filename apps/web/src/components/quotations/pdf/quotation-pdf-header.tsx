...
import { formatPdfDate, getPdfLogoUrl } from "./quotation-pdf-utils";
...
export function QuotationPdfHeader({ data }: Props) {
  const logoUrl = getPdfLogoUrl(data.business.logoUrl);
  ...
  return (
    <View style={styles.header}>
      ...
      {data.pdfOptions.showQuotationDetails && (
        <View style={styles.documentSection}>
          ...
          {!!data.revisionNumber && data.revisionNumber > 1 && (
            <Text style={styles.revision}>Revision {data.revisionNumber}</Text>
          )}

          <Text style={styles.date}>Date: {formatPdfDate(data.issueDate)}</Text>
